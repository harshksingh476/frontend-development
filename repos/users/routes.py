...
from repos.modules import User, Project, Log
...
def login():
    ...
    if request.method == "POST":
        ...
        if user and bcrypt.check_password_hash(user.password, password):
            ...
            message = f'Logged in to Traxx- user id {user.id}'
            log_info = Log(action=message, table_name='user', user_id=current_user.id,
...
@users.route('/logout')
def logout():
    message = f'Logged out from Traxx- user id {current_user.id}'
    log_info = Log(action=message, table_name='user', user_id=current_user.id,
                   action_time=datetime.datetime.now())
    db.session.add(log_info)
    db.session.commit()
